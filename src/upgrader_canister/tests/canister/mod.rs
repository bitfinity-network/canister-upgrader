...
async fn test_admin_can_manage_permissions() {
    ...
    // Act
    let permissions_before = client.admin_permissions_get(principal).await.unwrap().unwrap();
    let permissions_on_create = client.admin_permissions_add(principal, &[Permission::CreatePoll]).await.unwrap().unwrap();
    let permissions_after_create = client.admin_permissions_get(principal).await.unwrap().unwrap();
    let permissions_on_update = client.admin_permissions_add(principal, &[Permission::CreateProject]).await.unwrap().unwrap();
    let permissions_after_update = client.admin_permissions_get(principal).await.unwrap().unwrap();
    let permissions_on_remove = client.admin_permissions_remove(principal, &[Permission::CreatePoll]).await.unwrap().unwrap();
    let permissions_after_remove = client.admin_permissions_get(principal).await.unwrap().unwrap();

    // Assert
    assert_eq!(permissions_before.permissions.len(), 0);

    assert_eq!(permissions_on_create.permissions.len(), 1);
    assert!(permissions_on_create.permissions.contains(&Permission::CreatePoll));
    assert_eq!(permissions_after_create, permissions_on_create);

    assert_eq!(permissions_on_update.permissions.len(), 2);
    assert!(permissions_on_update.permissions.contains(&Permission::CreatePoll));
    assert!(permissions_on_update.permissions.contains(&Permission::CreateProject));
    assert_eq!(permissions_after_update, permissions_on_update);

    assert_eq!(permissions_on_remove.permissions.len(), 1);
    assert!(permissions_on_remove.permissions.contains(&Permission::CreateProject));
...
async fn test_only_admin_can_manage_permissions() {
    ...
    // Act & Assert
    assert!(client.admin_permissions_get(caller_principal).await.unwrap().is_err());
    assert!(client.admin_permissions_add(caller_principal, &[Permission::CreateProject]).await.unwrap().is_err());
    assert!(client.admin_permissions_remove(caller_principal, &[Permission::CreateProject]).await.unwrap().is_err());
...
async fn test_caller_can_get_own_permissions() {
        ...
        admin_client.admin_permissions_add(user_principal, &[Permission::CreatePoll]).await.unwrap().unwrap();
    
        // Act
        let user_permissions_from_admin = admin_client.admin_permissions_get(user_principal).await.unwrap().unwrap();
...
async fn test_caller_can_create_and_get_projects() {
        ...
        admin_client.admin_permissions_add(user_1_principal, &[Permission::CreateProject]).await.unwrap().unwrap();
        ...
        user_1_client.project_create(&project).await.unwrap().unwrap();
        ...
        let project_from_get = user_2_client.project_get(&project.key).await.unwrap().unwrap();
...
async fn test_caller_can_create_and_get_polls() {
            ...
            let user_1_principal = Principal::from_slice(&[1u8; 29]);
            let user_1_client = build_client(pocket.clone(), canister_principal, user_1_principal);
            admin_client.admin_permissions_add(user_1_principal, &[Permission::CreatePoll]).await.unwrap().unwrap();
...
async fn test_caller_can_vote_in_poll() {
            ...
            admin_client.admin_permissions_add(user_1_principal, &[Permission::CreatePoll, Permission::VotePoll]).await.unwrap().unwrap();
            admin_client.admin_permissions_add(user_2_principal, &[Permission::VotePoll]).await.unwrap().unwrap();
            ...
            // Act
            user_1_client.poll_vote(poll_id, false).await.unwrap().unwrap();
            user_2_client.poll_vote(poll_id, true).await.unwrap().unwrap();
